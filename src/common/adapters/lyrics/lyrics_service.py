...
class LyricsService(LoggingMixin):
    ...
    def __init__(self):
        ...
        # Genius API (opcional)
        self.genius_api_key = settings.GENIUS_CLIENT_SECRET
    ...
    async def get_lyrics(
        ...
        sources: List[Tuple[str, Callable[..., Awaitable[Optional[str]]]]] = [
        ...
        for source_name, source_func in sources:
            try:
                ...
                if source_name == "youtube":
                    if youtube_id:
                        lyrics = await source_func(
                            youtube_id, clean_title, clean_artist
                        )
                    else:
                        continue  # No hay youtube_id, saltar
                ...
                self.logger.warning(
                    f"Error obteniendo letras desde {source_name}: {str(e)}"
                )
                continue
    ...
    async def _get_lyrics_from_youtube(
        self, youtube_id: str, title: str, artist: str
    ) -> Optional[str]:
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                is_lyrics_video = any(
                    indicator in video_title for indicator in lyrics_indicators
                )
    ...
    async def _get_lyrics_from_lyrics_ovh(
    ...
    def _extract_lyrics_from_description(self, description: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.debug("Error extrayendo letras de la descripción")
                ...
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        lyrics_match = re.search(
                            r"<div[^>]*lyrics[^>]*>(.*?)</div>", html, re.DOTALL
                        )
                        if lyrics_match:
                            return self._clean_html(lyrics_match.group(1))
        except Exception:
            self.logger.debug("Error haciendo scraping de letras de Genius")
        return None
    ...
    def _extract_azlyrics_content(self, html: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.debug("Error extrayendo letras de AZLyrics")
