...
async def get_payment_methods(request):
    ...
    try:
        payment_methods = await get_payment_methods_use_case.execute(str(request.user.id))
...
async def get_invoice_history(request):
    ...
    try:
        ...
        invoices = await get_invoice_history_use_case.execute(str(request.user.id), limit)
