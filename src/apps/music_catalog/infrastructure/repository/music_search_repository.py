...
from apps.music_catalog.domain.repository.Imusic_repository import IMusicSearchRepository
from apps.music_catalog.domain.entities import SearchResultEntity, PaginatedResultEntity
...
class MusicSearchRepository(IMusicSearchRepository):
    ...
    def search_songs_paginated(self, query: str, page: int = 1, page_size: int = 20) -> PaginatedResultEntity:
        ...
        queryset = Song.objects.select_related('artist', 'album', 'genre').filter(
            Q(title__icontains=query) | Q(lyrics__icontains=query)
        ).filter(is_active=True).order_by('-play_count', 'title')
    ...
    def search_artists_paginated(self, query: str, page: int = 1, page_size: int = 20) -> PaginatedResultEntity:
        ...
        ).filter(is_active=True).order_by('-followers_count', 'name')
    ...
    def search_albums_paginated(self, query: str, page: int = 1, page_size: int = 20) -> PaginatedResultEntity:
        ...
        queryset = Album.objects.select_related('artist').filter(
            Q(title__icontains=query) | Q(description__icontains=query)
        ).filter(is_active=True).order_by('-release_date', 'title')
        
        return self._paginate_queryset(queryset, page, page_size, 'albums')
    
    def search_by_filters(self, filters: dict, page: int = 1, page_size: int = 20) -> PaginatedResultEntity:
        ...
        if 'duration_min' in filters and filters['duration_min']:
            queryset = queryset.filter(duration_seconds__gte=filters['duration_min'] * 60)
        
        if 'duration_max' in filters and filters['duration_max']:
            queryset = queryset.filter(duration_seconds__lte=filters['duration_max'] * 60)
    ...
    def get_paginated_songs(self, page: int = 1, page_size: int = 20) -> PaginatedResultEntity:
    ...
    def get_paginated_artists(self, page: int = 1, page_size: int = 20) -> PaginatedResultEntity:
        """Obtiene artistas paginados"""
        queryset = Artist.objects.filter(is_active=True).order_by('-followers_count', 'name')
        return self._paginate_queryset(queryset, page, page_size, 'artists')
    
    def get_paginated_albums(self, page: int = 1, page_size: int = 20) -> PaginatedResultEntity:
    ...
    def _search_genres(self, query: str) -> List[dict]:
        ...
        ).filter(is_active=True).order_by('-song_count', 'name')
        
        return [self._genre_to_dict(genre) for genre in genres]
    
    def _paginate_queryset(self, queryset, page: int, page_size: int, result_type: str) -> PaginatedResultEntity:
