...
class StatisticsAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        artists_data = {
            'labels': [artist.name[:30] + '...' if len(artist.name) > 30 else artist.name 
                      for artist in top_artists],
            'data': [artist.total_plays or 0 for artist in top_artists]
        }
        
        songs_data = {
            'labels': [f"{song.title[:25]}{'...' if len(song.title) > 25 else ''}" 
        ...
        stats_summary = {
            ...
            'total_plays': SongModel.objects.aggregate(
        ...
        return render(request, 'admin/statistics/statistics_dashboard.html', extra_context)
