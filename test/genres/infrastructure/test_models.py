...
def test_genre_model_creation():
    ...
    # Verificaciones
    assert genre_model.id == genre_data['id']
    assert genre_model.name == 'Rock'
    assert genre_model.description == 'Rock music genre'
    assert genre_model.image_url == 'https://example.com/rock.jpg'
    assert genre_model.color_hex == '#FF6B35'
    assert genre_model.popularity_score == 85
    assert genre_model.is_active == True
    assert genre_model.created_at is not None
    assert genre_model.updated_at is not None
...
def test_genre_model_minimal_data():
    ...
    # Verificaciones
    assert minimal_genre.name == 'Jazz'
    assert minimal_genre.description is None
    assert minimal_genre.image_url is None
    assert minimal_genre.color_hex is None
    assert minimal_genre.popularity_score == 0
    assert minimal_genre.is_active == True
    assert minimal_genre.created_at is not None
    assert minimal_genre.updated_at is not None
...
def test_genre_model_queries():
    ...
    assert len(active_genres) == 2
    assert len(inactive_genres) == 1
    
    # Test get específico
    rock_genre = MockGenreModel.objects_get(name='Rock')
    assert rock_genre.name == 'Rock'
    assert rock_genre.popularity_score == 85
...
def test_genre_model_updates():
    ...
    # Verificaciones
    assert genre.name == 'Classical Music'
    assert genre.popularity_score == 75
...
def test_model_to_entity_conversion():
    ...
    # Verificaciones
    assert isinstance(genre_entity, GenreEntity)
    assert genre_entity.name == 'Test Genre'
    assert genre_entity.description == 'Test description'
    assert genre_entity.color_hex == '#FF0000'
    assert genre_entity.popularity_score == 80
    assert genre_entity.is_active == True
...
def test_model_edge_cases():
    ...
    assert edge_genre.popularity_score == 100
    assert edge_genre.color_hex == '#000000'
