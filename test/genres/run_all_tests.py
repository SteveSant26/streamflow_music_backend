#!/usr/bin/env python3
"""
Script maestro para ejecutar todos los tests de Genres (Arquitectura Hexagonal)
"""
...
def run_test_file(test_file: str) -> tuple[bool, float]:
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, os.path.join("test", "genres", test_file)],
            capture_output=True,
            text=True,
            encoding="utf-8",
            env=env,
...
def main():
    """Función principal"""
    print("🎼 SUITE DE TESTS GENRES - ARQUITECTURA HEXAGONAL")
    print("=" * 60)
    ...
    test_files = [
        ...
        "use_cases/test_genre_use_cases.py",  # Tests de casos de uso
        "api/test_serializers.py",  # Tests de serializadores API
        "infrastructure/test_models.py",  # Tests de modelos/infrastructure
    ]
    ...
    # Resumen final
    print("\n" + "=" * 60)
    print("📊 RESUMEN FINAL DE TESTS - GENRES")
    print("=" * 60)
    ...
    for test_file, success in results.items():
        ...
        print(f"   {test_file:<30} {status} ({exec_time:.2f}s)")
    ...
    if failed == 0:
        print("\n🎉 ¡TODOS LOS TESTS DE GENRES PASARON!")
        print("✨ El módulo Genres está completamente probado y funcional")
    else:
