...
def test_get_genre_use_case():
    ...
    assert result.id == 'genre-123'
    assert result.name == 'Test Genre'
    assert result.description == 'Genre for testing'
    assert result.color_hex == '#FF0000'
    assert result.popularity_score == 75
...
def test_get_genre_not_found():
    ...
    try:
        ...
        assert False, "Debería haber lanzado GenreNotFoundException"
    except GenreNotFoundException as e:
        assert "nonexistent-genre" in str(e)
...
def test_get_all_genres_use_case():
    ...
    assert results[0].name == 'Rock'
    assert results[1].name == 'Pop'
    assert results[2].name == 'Jazz'
...
def test_get_popular_genres_use_case():
    ...
    assert results[0].name == 'Pop Genre 1'
    assert results[0].popularity_score == 95
...
def test_search_genres_by_name_use_case():
    ...
    # Verificaciones
    assert len(results) == 2
    assert 'Rock' in results[0].name or 'rock' in results[0].name.lower()
...
def test_edge_cases():
    ...
    # Test GetPopularGenresUseCase vacío
    use_case_popular = GetPopularGenresUseCase(mock_repository)
    results_popular = use_case_popular.execute()
    assert len(results_popular) == 0
    
    # Test SearchGenresByNameUseCase vacío
