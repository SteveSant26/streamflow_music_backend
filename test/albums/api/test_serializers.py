...
def create_mock_album_entity(album_id: str = None, title: str = "Test Album") -> AlbumEntity:
...
def test_album_serializer_validation():
    ...
    assert serializer.validated_data['title'] == 'Dark Side of the Moon'
    assert serializer.validated_data['total_tracks'] == 10
...
def test_album_serializer_validation_errors():
    ...
    # Datos incompletos - falta artist_id
    invalid_data2 = {
        'title': 'Test Album',
        'artist_name': 'Pink Floyd'
    }
    ...
    assert serializer2.is_valid() == False
...
def test_album_serializer_representation():
    ...
    album = create_mock_album_entity(
        album_id="album-123",
        title="The Wall"
    )
    ...
    # Verificaciones
    assert representation['id'] == "album-123"
    assert representation['title'] == "The Wall"
    assert representation['artist_name'] == "Test Artist"
    assert representation['total_tracks'] == 10
    assert representation['play_count'] == 5000
    assert representation['is_active'] == True
    assert representation['release_date'] == "2020-01-01"
...
def test_album_serializer_minimal_data():
    ...
    # Verificaciones
    assert representation['id'] == "minimal-123"
    assert representation['title'] == "Minimal Album"
    assert representation['artist_name'] == "Minimal Artist"
    assert representation['release_date'] is None
    assert representation['description'] is None
    assert representation['cover_image_url'] is None
    assert representation['total_tracks'] == 0
    assert representation['play_count'] == 0
    assert representation['is_active'] == True
...
def test_album_search_serializer():
    ...
    assert search_serializer.is_valid() == True
    assert search_serializer.validated_data['q'] == 'rock'
    
    # Datos de búsqueda inválidos (sin query)
    invalid_search_data = {
        'artist_name': 'Pink Floyd'
    }
    
    invalid_search_serializer = MockAlbumSearchSerializer(data=invalid_search_data)
    assert invalid_search_serializer.is_valid() == False
...
def test_album_response_serializer():
    ...
    # Verificaciones
    assert response_repr['source'] == 'local_cache'
    assert len(response_repr['results']) == 2
    assert response_repr['results'][0]['title'] == "Result Album 1"
    assert response_repr['results'][1]['title'] == "Result Album 2"
    assert response_repr['message'] == 'Found 2 albums'
