...
def test_album_model_creation():
    ...
    assert album.is_active == True  # Valor por defecto
...
def test_album_model_minimal_data():
    ...
    assert album.artist_name == 'The Beatles'
    assert album.release_date is None
    assert album.description == ''
    assert album.cover_image_url == ''
    assert album.total_tracks == 0  # Valor por defecto
    assert album.play_count == 0  # Valor por defecto
    assert album.is_active == True  # Valor por defecto
...
def test_album_model_queries():
    ...
    assert all_albums.count() >= 3
    assert active_albums.count() >= 2
    assert inactive_albums.count() >= 1
    
    # Test filtro por título
    wall_albums = MockAlbumModel.objects.filter(title__icontains='Wall')
    assert wall_albums.count() >= 1
    assert wall_albums.first().title == 'The Wall'
...
def test_model_to_entity_conversion():
    ...
    assert entity.artist_name == model.artist_name
    assert entity.total_tracks == model.total_tracks
    assert entity.play_count == model.play_count
