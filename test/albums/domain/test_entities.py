...
def test_album_entity_creation():
    ...
    # Verificaciones
    assert album.id == album_data["id"]  # nosec B101
...
def test_album_entity_minimal_data():
    ...
    # Verificaciones
    assert album.id == minimal_data["id"]  # nosec B101
    assert album.title == "Abbey Road"  # nosec B101
    assert album.artist_id == minimal_data["artist_id"]  # nosec B101
    assert album.artist_name is None  # nosec B101
    assert album.release_date is None  # nosec B101
    assert album.description is None  # nosec B101
...
def test_album_entity_properties():
    ...
    assert album.title == "The Wall"  # nosec B101
    assert album.total_tracks == 26  # nosec B101
    assert album.release_date.year == 1979  # nosec B101
    assert album.artist_name == "Pink Floyd"  # nosec B101
...
def test_album_entity_string_representation():
    ...
    str_repr = str(album)
    assert "Thriller" in str_repr  # nosec B101
    assert "album-123" in str_repr or "Michael Jackson" in str_repr  # nosec B101
...
def test_album_entity_equality():
    ...
    album1 = AlbumEntity(
        id=album_id, title="Hotel California", artist_id=artist_id, artist_name="Eagles"
    )
    ...
    assert album1.id == album2.id  # nosec B101
    assert album1.id != album3.id  # nosec B101
