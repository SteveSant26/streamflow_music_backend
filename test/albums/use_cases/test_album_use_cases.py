...
def create_mock_album(album_id: str, title: str, artist_name: str = 'Test Artist', 
                     total_tracks: int = 10, play_count: int = 1000) -> AlbumEntity:
...
def test_get_album_not_found():
    ...
    try:
        ...
        assert False, "Debería haber lanzado AlbumNotFoundException"
    except AlbumNotFoundException as e:
        assert "nonexistent-album" in str(e)
...
def test_get_all_albums_use_case():
    ...
    assert results[0].title == 'Album One'
    assert results[1].title == 'Album Two'
    assert results[2].title == 'Album Three'
    assert results[0].total_tracks == 10
...
def test_search_albums_by_title_use_case():
    ...
    assert results[0].title == 'Rock Album 1'
    assert results[1].title == 'Rock Album 2'
...
def test_get_albums_by_artist_use_case():
    ...
    artist_albums = [
        create_mock_album('album-1', 'First Album', 'Pink Floyd', total_tracks=8, play_count=50000),
        create_mock_album('album-2', 'Second Album', 'Pink Floyd', total_tracks=10, play_count=80000)
    ]
    ...
    # Verificaciones
    assert len(results) == 2
    assert all(album.artist_name == 'Pink Floyd' for album in results)
    assert results[0].title == 'First Album'
