...
project_root = os.path.dirname(current_dir)  # Subir un nivel desde test/ para llegar a la raíz
...
def print_banner():
    ...
    print(Colors.HEADER + Colors.BOLD + "=" * 80)
    print("🎵 STREAMFLOW MUSIC BACKEND - MASTER TEST SUITE 🎵")
    print("=" * 80 + Colors.ENDC)
    print(f"{Colors.OKCYAN}📅 Iniciado: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Colors.ENDC}")
    print(f"{Colors.OKBLUE}🏗️  Arquitectura: Hexagonal{Colors.ENDC}")
    print(f"{Colors.OKBLUE}🧪 Testing: Completo en 6 Módulos{Colors.ENDC}")
...
def run_module_tests(module_name: str) -> Tuple[bool, float, Dict]:
    ...
    test_runner_path = os.path.join(project_root, "test", module_name, "run_all_tests.py")
    
    print(f"   🔍 Buscando runner en: {test_runner_path}")
    
    if not os.path.exists(test_runner_path):
        print(f"{Colors.WARNING}⚠️  Test runner no encontrado para {module_name}{Colors.ENDC}")
    ...
    try:
        ...
        if success:
            print(f"{Colors.OKGREEN}✅ {module_name.upper()} completado exitosamente ({execution_time:.2f}s){Colors.ENDC}")
        else:
            print(f"{Colors.FAIL}❌ {module_name.upper()} falló (código: {result.returncode}){Colors.ENDC}")
            ...
            if 'pasaron' in line and 'fallaron' in line:
                ...
                if len(parts) > 1:
                    ...
                    if 'fallaron' in results_part:
                        failed_part = results_part.split('fallaron')[0].split('pasaron')[1].strip()
                        stats['tests_failed'] = int(failed_part.replace(',', '').strip().split()[0])
...
def print_final_summary(results: Dict[str, Tuple[bool, float, Dict]]):
    ...
    for module_name, (success, exec_time, stats) in results.items():
        ...
        print(f"   {status_color}{status_icon} {module_name.ljust(15)} {status_text.ljust(8)} ({exec_time:.2f}s){tests_info}{Colors.ENDC}")
    ...
    if failed_modules == 0:
        print(f"{Colors.OKGREEN}{Colors.BOLD}🎉 ¡TODOS LOS MÓDULOS PASARON EXITOSAMENTE!{Colors.ENDC}")
        print(f"{Colors.OKGREEN}✨ StreamFlow Music Backend está completamente probado y funcional{Colors.ENDC}")
        print(f"{Colors.OKGREEN}🚀 El proyecto está listo para producción desde la perspectiva de testing{Colors.ENDC}")
        ...
        print(f"{Colors.WARNING}⚠️  {failed_modules} módulo(s) tuvieron problemas{Colors.ENDC}")
