    ...
    def test_extract_album_from_description(self):
        ...
        with patch.object(self.extractor, "_extract_artists") as mock_extract_artists:
            mock_extract_artists.return_value = [
                ExtractedArtistInfo(
                    name="Taylor Swift", extracted_from="channel", confidence_score=0.8
                )
            ]
...
class TestMusicIntegrationService:
    ...
    async def test_get_random_music_with_metadata_success(self):
        ...
        mock_videos = [
            YouTubeVideoInfo(
                ...
                extracted_artists=[
                    ExtractedArtistInfo(
                        name="Test Artist", extracted_from="title", confidence_score=0.8
                    )
                ],
            )
        ]
    ...
    async def test_process_extracted_artists_found_in_db(self):
        ...
        # Ejecutar
        result = await self.integration_service._process_extracted_artists(
            videos, False
        )
    ...
    async def test_create_artist_from_extraction(self):
        ...
        artist_info = ExtractedArtistInfo(
            name="New Artist", extracted_from="title", confidence_score=0.7
        )
        ...
        # Ejecutar
        result = await self.integration_service._create_artist_from_extraction(
            artist_info
        )
    ...
    def test_calculate_artist_match_confidence(self):
        ...
        # Match exacto
        confidence = self.integration_service._calculate_artist_match_confidence(
            db_artist, extracted_artist
        )
    ...
    def test_generate_integration_stats(self):
        ...
        videos = [
            YouTubeVideoInfo(
                ...
                extracted_artists=[
                    ExtractedArtistInfo(
                        name="Artist", extracted_from="title", confidence_score=0.8
                    )
                ],
                extracted_albums=[
                    ExtractedAlbumInfo(
                        title="Album", extracted_from="title", confidence_score=0.7
                    )
                ],
            )
        ]
        ...
        stats = self.integration_service._generate_integration_stats(
            videos, artists_info, albums_info
        )
