...
class MockSearchIndex:
    ...
    def search_artists(self, query: str, limit: int = 10) -> List[dict]:
        ...
        for artist in self.artists_data:
            if (
                query_lower in artist["name"].lower()
                or query_lower in artist["genre"].lower()
            ):
    ...
    def search_albums(self, query: str, limit: int = 10) -> List[dict]:
        ...
        for album in self.albums_data:
            if (
                query_lower in album["title"].lower()
                or query_lower in album["artist"].lower()
                or query_lower in album["genre"].lower()
            ):
    ...
    def search_genres(self, query: str, limit: int = 10) -> List[dict]:
        ...
        for genre in self.genres_data:
            if (
                query_lower in genre["name"].lower()
                or query_lower in genre["description"].lower()
            ):
...
def test_search_index_creation():
    ...
    assert len(index.artists_data > 0)  # nosec B101
    assert len(index.albums_data > 0)  # nosec B101
    assert len(index.songs_data > 0)  # nosec B101
    assert len(index.genres_data > 0)  # nosec B101
...
def test_search_index_artists_search():
    ...
    assert results[0]["name"] == "Queen"  # nosec B101
    assert results[0]["genre"] == "Rock"  # nosec B101
    rock_results = index.search_artists("Rock")
    assert len(rock_results >= 2)  # nosec B101

    # Buscar algo que no existe
    no_results = index.search_artists("NonExistentArtist")
    assert len(no_results == 0)  # nosec B101
...
def test_search_index_songs_search():
    ...
    assert len(results == 1)  # nosec B101
    assert results[0]["title"] == "Bohemian Rhapsody"  # nosec B101
    assert results[0]["artist"] == "Queen"  # nosec B101
    queen_songs = index.search_songs("Queen")
    assert len(queen_songs >= 2)  # nosec B101
...
def test_search_repository_creation():
    ...
    assert repo.search_index is not None  # nosec B101
    assert isinstance(repo.search_index, MockSearchIndex)  # nosec B101
...
def test_search_repository_artists():
    ...
    assert len(results == 1)  # nosec B101
    assert isinstance(results[0], SearchResultEntity)  # nosec B101
    assert results[0].result_type == "artist"  # nosec B101
    assert results[0].title == "Queen"  # nosec B101
    assert results[0].subtitle == "Género: Rock"  # nosec B101
    assert results[0].relevance_score > 0.0  # nosec B101
...
def test_search_repository_songs():
    ...
    assert len(results == 1)  # nosec B101
    assert isinstance(results[0], SearchResultEntity)  # nosec B101
    assert results[0].result_type == "song"  # nosec B101
    assert results[0].title == "Bohemian Rhapsody"  # nosec B101
    assert results[0].subtitle == "Queen"  # nosec B101
    assert results[0].relevance_score > 0.0  # nosec B101
...
def test_search_history_repository():
    ...
    assert len(history_repo.history == 3)  # nosec B101
    user_history = history_repo.get_user_history("user-123")
    assert len(user_history == 2)  # nosec B101
    assert all(q.user_id == "user-123" for q in user_history)  # nosec B101
    user2_history = history_repo.get_user_history("user-456")
    assert len(user2_history == 1)  # nosec B101
    assert user2_history[0].query_text == "jazz classics"  # nosec B101
...
def test_search_history_clear():
    ...
    assert cleared_count == 2  # nosec B101
    assert len(history_repo.history == 1)  # nosec B101
    assert history_repo.history[0].user_id == "user-456"  # nosec B101
...
def test_infrastructure_edge_cases():
    ...
    try:
        ...
        assert len(empty_results == 0)  # nosec B101
        zero_limit_results = repo.search_artists("Queen", limit=0)
        assert len(zero_limit_results == 0)  # nosec B101
        lower_results = repo.search_artists("queen", limit=5)
        upper_results = repo.search_artists("QUEEN", limit=5)
        assert len(lower_results == len(upper_results))  # nosec B101
        assert len(lower_results > 0)  # nosec B101
