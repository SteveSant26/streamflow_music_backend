...
class UniversalSearchUseCase:
    ...
    def __init__(
        ...
        history_repo: ISearchHistoryRepository = None,
    ):
        self.repository = repository
        self.history_repo = history_repo

    def execute(
        self, query: str, types: List[str] = None, limit: int = 10, user_id: str = None
    ) -> SearchResponseEntity:
        ...
        artists = (
            self.repository.search_artists(query, limit) if "artists" in types else []
        )
        albums = (
            self.repository.search_albums(query, limit) if "albums" in types else []
        )
        songs = self.repository.search_songs(query, limit) if "songs" in types else []
        genres = (
            self.repository.search_genres(query, limit) if "genres" in types else []
        )
...
def test_universal_search_use_case():
    ...
    albums = [
        SearchResultEntity(
            "album", "al1", "A Night at the Opera", "Queen", relevance_score=0.85
        )
    ]
    songs = [
        SearchResultEntity(
        ...
        SearchResultEntity(
            "song", "s2", "We Will Rock You", "Queen", relevance_score=0.88
        ),
    ]
    ...
    # Verificaciones
    assert result.query == "queen"  # nosec B101
    assert len(result.artists == 1)  # nosec B101
    assert len(result.albums == 1)  # nosec B101
    assert len(result.songs == 2)  # nosec B101
    assert len(result.genres == 1)  # nosec B101
    assert result.total_results == 5  # nosec B101
    assert result.search_time_ms is not None  # nosec B101
    assert result.search_time_ms > 0  # nosec B101
...
def test_universal_search_with_filters():
    ...
    mock_repository.search_songs.return_value = [
        SearchResultEntity(
            "song", "s1", "Test Song", "Test Artist", relevance_score=0.8
        )
    ]
    ...
    # Verificaciones
    assert len(result.artists == 0)  # nosec B101
    assert len(result.albums == 0)  # nosec B101
    assert len(result.songs == 1)  # nosec B101
    assert len(result.genres == 0)  # nosec B101
    assert result.total_results == 1  # nosec B101
...
def test_universal_search_with_history():
    ...
    assert call_args.query_text == "test query"  # nosec B101
    assert call_args.user_id == "user-123"  # nosec B101
    assert call_args.results_count == 1  # nosec B101
...
def test_universal_search_empty_query():
    ...
    try:
        use_case.execute("")
        assert False, "Debería haber lanzado EmptyQueryException"  # nosec B101
    except EmptyQueryException as e:
        assert "empty" in str(e.lower())  # nosec B101
...
def test_quick_search_use_case():
    ...
    mock_repository.search_songs.return_value = [
        SearchResultEntity(
            "song", "s1", "Quick Song 1", "Artist 1", relevance_score=0.9
        ),
        SearchResultEntity(
            "song", "s2", "Quick Song 2", "Artist 2", relevance_score=0.8
        ),
    ]
    ...
    assert results[0].title == "Quick Song 1"  # nosec B101
    assert results[1].title == "Quick Song 2"  # nosec B101
...
def test_quick_search_empty_query():
    ...
    assert len(results == 0)  # nosec B101
...
def test_search_history_use_case():
    ...
    # Verificaciones
    assert len(results == 2)  # nosec B101
    assert results[0].query_text == "rock music"  # nosec B101
    assert results[1].query_text == "pop songs"  # nosec B101
