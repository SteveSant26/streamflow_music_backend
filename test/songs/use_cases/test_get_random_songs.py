...
class TestGetRandomSongs(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_random_songs_from_database_insufficient(self):
        ...
        self.song_use_cases.music_service.get_random_music_tracks.return_value = new_tracks
        ...
        with patch.object(self.song_use_cases, '_save_track_as_song', return_value=new_song):
            # Act
            result = await self.song_use_cases.get_random_songs(count=count)
            
            # Assert
            self.assertGreaterEqual(len(result), 1)  # Al menos debe retornar algo
            self.song_use_cases.music_service.get_random_music_tracks.assert_called_once_with(count)
    ...
    async def test_get_random_songs_force_refresh(self):
        ...
        with patch.object(self.song_use_cases, '_save_track_as_song', return_value=new_song):
            # Act
            result = await self.song_use_cases.get_random_songs(count=count, force_refresh=True)
    ...
    async def test_get_random_songs_existing_youtube_song(self):
        ...
        self.song_use_cases.music_service.get_random_music_tracks.return_value = new_tracks
        self.mock_repository.get_by_source.return_value = existing_song  # Canción ya existe
    ...
    async def test_get_random_songs_error_handling(self):
        ...
        self.song_use_cases.music_service.get_random_music_tracks.side_effect = Exception("YouTube API Error")
        self.mock_repository.get_random.side_effect = [
