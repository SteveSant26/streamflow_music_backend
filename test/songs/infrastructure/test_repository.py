...
class TestSongRepository(TestCase):
    ...
    async def test_save_new_song(self):
        ...
        # Arrange
        with patch(
            "apps.songs.infrastructure.repository.song_repository.sync_to_async"
        ) as mock_sync:
    ...
    async def test_search_songs_concept(self):
        ...
        artist_songs = Song.objects.filter(
            artist_name=self.sample_model_data["artist_name"]
        )
    ...
    async def test_get_by_source_concept(self):
        ...
        found_song = Song.objects.filter(
            source_type="youtube", source_id="unique-youtube-id"
        ).first()
