...
class TestSongSerializer(unittest.TestCase):
    ...
    def test_serialize_song_missing_required_fields(self):
        ...
        required_fields = ['title', 'youtube_video_id', 'duration_seconds', 
                          'youtube_url', 'play_count', 'youtube_view_count', 
                          'youtube_like_count', 'is_explicit', 'audio_downloaded']
    ...
    def test_serialize_song_invalid_data_types(self):
        ...
        for field, invalid_value, description in test_cases:
            with self.subTest(field=field, value=invalid_value):
                ...
                serializer = SongSerializer(data=data)
                self.assertFalse(serializer.is_valid(), f"Field {field} should be invalid: {description}")
                self.assertIn(field, serializer.errors)
...
class TestSongListSerializer(unittest.TestCase):
    ...
    def test_compare_serializers_field_subset(self):
        ...
        for field_name in common_fields:
            if field_name != 'duration_formatted':  # Este es SerializerMethodField en ambos
