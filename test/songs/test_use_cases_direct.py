...
async def test_get_random_songs():
    ...
    assert result[2].title == 'Test Song 3'
    
    # Verificar que se llamó el repositorio
...
async def test_search_songs():
    ...
    # Verificaciones
    assert len(result) == 2
    assert result[0].title == 'Rock Song'
    assert result[1].title == 'Another Rock Song'
    assert all('Rock' in song.title for song in result)
...
async def test_use_cases_with_empty_results():
    ...
    assert len(random_result) == 0
    
    # Test search_songs vacío
