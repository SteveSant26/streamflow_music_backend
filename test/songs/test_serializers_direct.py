...
from apps.songs.api.serializers.song_serializers import SongSerializer, SongListSerializer
...
def test_song_serializer():
    ...
    assert validated_data['title'] == 'Test Serializer Song'
    assert validated_data['youtube_video_id'] == 'abc123def'
    assert validated_data['artist_name'] == 'Serializer Artist'
    assert validated_data['duration_seconds'] == 240
    assert validated_data['tags'] == ['rock', 'test', 'serializer']
    assert validated_data['is_explicit'] == False
    assert validated_data['audio_downloaded'] == True
...
def test_song_serializer_duration_formatted():
    ...
    for duration, expected_format in test_cases:
        ...
        assert actual_format == expected_format, f"Duration {duration} should format as {expected_format}, got {actual_format}"
...
def test_song_list_serializer():
    ...
    # Verificar datos
    validated_data = serializer.validated_data
    assert validated_data['title'] == 'List Test Song'
    assert validated_data['artist_name'] == 'List Artist'
    assert validated_data['album_title'] == 'List Album'
    assert validated_data['play_count'] == 25
...
def test_song_serializer_validation():
    ...
    assert 'youtube_video_id' in serializer.errors
    ...
    assert is_valid_negative == False
    assert 'duration_seconds' in serializer_negative.errors
    ...
    assert 'youtube_url' in serializer_url.errors
...
def test_song_serializer_nullable_fields():
    ...
    validated_data = serializer.validated_data
    assert validated_data['artist_name'] is None
    assert validated_data['album_title'] is None
    assert validated_data['genre_name'] is None
    assert validated_data['file_url'] is None
    assert validated_data['thumbnail_url'] is None
