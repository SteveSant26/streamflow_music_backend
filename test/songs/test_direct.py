...
def test_song_entity_creation():
    ...
    # Verificaciones
    assert song.id == "song-test-123"  # nosec B101
    assert song.title == "Test Song"  # nosec B101
    assert song.artist_name == "Test Artist"  # nosec B101
    assert song.duration_seconds == 180  # nosec B101
...
def test_song_entity_minimal():
    ...
    assert song.id == "minimal-song"  # nosec B101
    assert song.title == "Minimal Test"  # nosec B101
...
def test_song_entity_tags():
    ...
    song_with_tags = SongEntity(
        id="tagged-song", title="Tagged Song", tags=["rock", "classic"]
    )

    assert song_with_tags.tags == ["rock", "classic"]  # nosec B101
    song_no_tags = SongEntity(id="untagged-song", title="Untagged Song")

    assert song_no_tags.tags == []  # nosec B101
